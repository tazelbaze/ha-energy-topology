...
class EnergyTopologyPanel extends HTMLElement {
  ...
  async _load() {
    ...
    this._loading = true;
    try {
      this._apply(await this._hass.callWS({ type: "energy_topology/get" }));
      this._loaded = true;
    } catch (err) {
  ...
  _control(node) {
    ...
    if (node.manual_panel) {
      return `<button class="mark active" data-id="${ESC(node.id)}" data-panel="false" title="Retirer la marque tableau">tableau ✓</button>`;
    }
    return `<button class="mark" data-id="${ESC(node.id)}" data-panel="true" title="Marquer comme tableau">+ tableau</button>`;
  }
  ...
  _render() {
    ...
    const hint = this._isAdmin
      ? `<p class="hint">Astuce : marquez un sous-compteur sans enfant (ex. un étage) comme <strong>tableau</strong> pour qu'il apparaisse comme zone.</p>`
      : "";
    ...
    this.querySelectorAll(".mark").forEach((btn) => btn.addEventListener("click", (event) => {
  ...
  _styles() { return `
    ...
    button,input{font:inherit;border:1px solid var(--divider-color);border-radius:10px;padding:10px 14px;background:var(--card-background-color);color:inherit} #refresh{cursor:pointer;background:var(--primary-color);color:var(--text-primary-color)}
    ...
    .metric-error{border-color:var(--error-color)!important}.toolbar input{width:100%;box-sizing:border-box}.hint{font-size:13px;margin:8px 2px 0}
    .banner:not(:empty){margin-top:12px;padding:10px 14px;border-radius:10px;border:1px solid var(--error-color);color:var(--error-color)}
    ...
    button.mark{font-size:11px;padding:2px 9px;border-radius:999px;cursor:pointer;background:transparent;color:var(--secondary-text-color)}button.mark:hover{border-color:var(--primary-color);color:var(--primary-color)}button.mark.active{border-color:var(--primary-color);color:var(--primary-color)}
